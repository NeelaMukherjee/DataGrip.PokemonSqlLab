...
from pokemons p JOIN types t ON p.secondary_type = t.id AND p.name = "Rufflet";

#What are the names of the pokemon that belong to the trainer with trainerID 303?
select p.name
from pokemon_trainer pk JOIN pokemons p ON pk.pokemon_id = p.id AND pk.trainerID = 303;

#How many pokemon have a secondary type Poison
select count(*)
from types t JOIN pokemons p ON t.id = p.secondary_type AND t.name = "Poison";


#What are all the primary types and how many pokemon have that type?
select t.name as PRIMARY_TYPES, COUNT(*) as POKEMON_COUNT
from types t LEFT JOIN pokemons p  ON  t.id =p.primary_type group by  t.name
...
select count(*)
from pokemon_trainer pk JOIN pokemons p ON pk.pokemon_id = p.id AND pk.pokelevel = 100
...
select p.name as Pokemon_Name, t.trainername as Trainer_Name, pk.pokelevel as Level,
from pokemon_trainer pk, pokemons p, trainers as tr , types as ty ON pk.pokemon_id=p.id AND pk.trainerID = t.trainerID AND  ty.id = p.prima